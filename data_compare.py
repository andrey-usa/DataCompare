...
class ResultsManager:
    ...
    OUTPUT_CONFIGS = [
        ...
        OutputConfig("unpivoted_mismatches", "Value_Unpivoted_Mismatches_{file1_name}_vs_{file2_name}.csv", "Open Value (unpivoted) Mismatches", 4),
    ...
    def get_output_buttons_config(self, saved_files: List[str]) -> List[Tuple[str, str]]:
        ...
        saved_files_map = {os.path.basename(p): p for p in saved_files}
        buttons_config = []
        ...
        for config in sorted(self.OUTPUT_CONFIGS, key=lambda x: x.order):
            ...
            if filename in saved_files_map:
                button_text = config.button_text_template.format(
                    file1_name=self.file1_name,
                    file2_name=self.file2_name
                )
...
class DataCompareApp(TkinterDnD.Tk):
    ...
    def _create_widgets(self):
        ...
        self.compare_button = ttk.Button(compare_button_frame, text="Compare Files", command=self._run_comparison, style='Accent.TButton')
        self.compare_button.pack(pady=5)

        # --- Dashboard Section ---
        dashboard_frame = ttk.LabelFrame(middle_frame, text="2. Comparison Results", padding=(10, 5))
        dashboard_frame.pack(fill='x', expand=True)
        ...
        self.save_button = ttk.Button(save_button_frame, text="Save Results", command=self._save_results, state='disabled')
    ...
    def _create_dashboard_widgets(self, parent_frame):
        ...
        stats_config = [
            ...
            ("missing_in_file2", "Missing in File 1:"),
            ("missing_in_file1", "Missing in File 2:"),
        ...
        for i, (key, text) in enumerate(stats_config):
    ...
    def _browse_file(self, var):
        ...
        path = filedialog.askopenfilename(
            filetypes=[('All Supported', '*.xlsx;*.xls;*.csv'), ('Excel Files', '*.xlsx;*.xls'), ('CSV Files', '*.csv'), ('All Files', '*.*')]
        )
        if path:
            var.set(path)
            self._suggest_key_columns()
    ...
    def _update_gui_with_results(self, results):
        ...
        self.stat_values["file1_stats"].set(f"{df1_shape[0]} rows, {df1_shape[1]} columns")
        self.stat_values["file2_stats"].set(f"{df2_shape[0]} rows, {df2_shape[1]} columns")
        self.stat_values["missing_in_file2"].set(f"{len(missing_in_file1)} rows")
        self.stat_values["missing_in_file1"].set(f"{len(missing_in_file2)} rows")
        self.stat_values["mismatches"].set(f"{len(mismatches)} rows")
        self.stat_values["duplicates_file1"].set(f"{len(duplicates_file1)} rows")
        self.stat_values["duplicates_file2"].set(f"{len(duplicates_file2)} rows")
        self.stat_values["unpivoted_mismatches"].set(f"{len(unpivoted_mismatches)} rows")
        ...
        self.stat_labels["missing_in_file2"].config(text=f"Missing in '{f1_name}':")
        self.stat_labels["missing_in_file1"].config(text=f"Missing in '{f2_name}':")
    ...
    def _save_results(self):
        ...
        try:
            ...
            if saved_files:
                ttk.Label(self.output_frame, text=f"Results saved to: {os.path.realpath(output_folder)}").pack(anchor='w', pady=(0, 5))
